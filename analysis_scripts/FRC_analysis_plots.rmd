...
###### 2. 400x400 bounding box through full z.
###### 3. Fuse (no downsampling, keeping anisotropy) and save.
###### 4. Create new .hdf5 based on this small image and open in new project
###### 5. FRC (no downsampling) with relative FRC, **FFT size 200**, **stepsize=1** and **not keeping anisotropy** (FFT size 400 for confocal data)
###### - this will create 4 sub-tiles where FRC is calculated independently
###### - to assess FRC quality we always take the median of these 4 subtiles
###### 6. For both Img and FRC measurements: Measurement per slice.
...
read_data <- function(files){
  ...
  mutate(angle=ifelse(angle == 0, "right",angle)) %>%
  mutate(angle=ifelse(angle == 1,"left",angle)) %>%
  #if angle is a digit number keep it otherwise make it NA
...
data_full <- map_dfr(file.path(here("data"),list_data_csvs),read_data) %>% 
  ...
  select(protocol,orgID,slice,angle,measured, value,max_Img,min_Img) %>%
  ...
  group_by(protocol,orgID,angle,measured) %>%
  ...
  group_by(slice,orgID,protocol,angle) %>%
  ungroup() %>%
...
FRC_plot <- data_to_plot %>%
  ...
  filter(value > 0) %>% 
  line_plot(.,"slice_norm_micro","value","µm (relative to center)","FRC score (raw)","name") +
  facet_grid(~angle,scale="free_x")
...
FRC_plot <- data_to_plot %>%
  filter(measured == "FRC") %>% 
  line_plot(.,"microns_from_start","value_norm","µm inside","FRC score (normalized)","replicates")+
    geom_hline(yintercept=0.5,linetype='dashed',size=0.8,color="black",alpha=0.7)
...
boxplot_FRC <- ggplot(data_to_plot2_FRC, aes(y=value,x=protocol,group=protocol,fill=protocol))+
  ...
  geom_signif(comparisons = list(c("Fructose", "ScaleA2"),c("ClearT2","Fructose")), 
              map_signif_level=TRUE,y_position = c(74, 81),test = "wilcox.test",size=1)+
  geom_signif(comparisons = list(c("ClearT2","ScaleA2")),
              map_signif_level = TRUE,y_position=c(88),test = "wilcox.test",size=1)+
...
boxplot_DCT_Shannon_Entropy <- ggplot(data_to_plot2_DCT_Shannon_entropy, aes(y=value,x=protocol,group=protocol,fill=protocol))+
  ...
  # geom_signif(comparisons = list(c("Fructose", "ScaleA2"),c("ClearT2","Fructose")), 
  #             map_signif_level=TRUE,y_position = c(0.029, 0.031),test = "wilcox.test",size=1)+
  # geom_signif(comparisons = list(c("ClearT2","ScaleA2")),
              # map_signif_level = TRUE,y_position=c(0.033),test = "wilcox.test",size=1)+
